...
class Wallet <Currency extends CurrencyCode> { 
  ...
  transferTo<NewCurrency extends CurrencyCode>(newCurrency: NewCurrency) {
    const newStored = this.stored / conversionRates.usd * conversionRates[newCurrency as keyof typeof conversionRates];
...
function purchaseInCurrency<Currency extends CurrencyCode>(wallet: Wallet<Currency>, tag: PriceTag<Currency>) {
